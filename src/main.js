import './style.css';
import { categorias } from './main.ts';
...
const atualizarListaButton = document.getElementById('atualizar-lista');
const privacidadePopup = document.getElementById('privacidade-popup');
const fecharPrivacidadeButton = document.getElementById('fechar-privacidade');

function mostrarAvisoPrivacidade() {
...
function categoriaValida(valor) {
    return categorias.includes(valor);
}
...
function carregarDespesas() {
    ...
    try {
        ...
        return despesasSalvas.filter((despesa) => {
            if (!despesa || typeof despesa !== 'object') {
                return false;
            }

            return typeof despesa.id === 'string'
                && typeof despesa.titulo === 'string'
                && typeof despesa.valor === 'number'
                && Number.isFinite(despesa.valor)
                && categoriaValida(despesa.categoria ?? '')
                && (typeof despesa.pago === 'boolean' || typeof despesa.pago === 'undefined');
...
function obterTotaisPorCategoria() {
    const totais = Object.fromEntries(categorias.map((categoria) => [categoria, 0]));
...
function focarCampoComErro(campo, mensagem) {
    mostrarErro(mensagem);
    campo.setAttribute('aria-invalid', 'true');
    campo.focus();
}

function converterValorDespesa(valorDigitado) {
